...
## 2026-05-10 13:12:59 +09:00 - Step 4 Redis 비동기 대기열 적용 기록 분리

### 1. 발생 상황
- Step 4 Redis 비동기 대기열 적용 내역은 Step 3 분산 락과 비교할 수 있도록 별도 변경 로그로 관리할 필요가 있다.

### 2. 원인
- Redisson 분산 락과 Redis ZSET 대기열은 모두 Redis를 사용하지만, 요청 처리 모델이 동기 처리와 비동기 처리로 완전히 다르다.
